...
### ✅ G6 — 辯論的門檻與輪數上限沒有回傳（**2026-09-07 已修並驗過**）

`get_debate` 與 `list_debates` 都回傳 `drift_max: 0.5` / `max_rounds: 10`。
前端寫死門檻是最糟的做法——那是政策值的第二份拷貝，不一致時不會報錯。

以下是原始的問題紀錄：

### G6（原始）— 辯論的門檻與輪數上限沒有回傳
...
### ✅ G7 — 沒有「這個專案辯了哪些方向」的端點（**2026-09-07 已修並驗過**）

`GET /compute/debate?project_id=` 回傳該專案辯過的方向摘要，照錦標賽名次排。
實測胰臟癌專案回 `n_debated: 3`，名次 2/4/6，各帶漂移、未解決反對、終止原因。

以下是原始的問題紀錄：

### G7（原始）— 沒有「這個專案辯了哪些方向」的端點
