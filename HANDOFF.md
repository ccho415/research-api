...
## ✅ 階段死在半路時終於有人回報（2026-09-12，含實測）

### 這修的是 2026-09-10 那個坑的根因

六支階段工作流的 `Tell The Chain` 都排在自己的迴圈**之後**，所以迴圈裡死掉就
整個跳過——`run` 那列停在 `running`，migration 017 不准同階段排兩次，鏈永遠
不動，而進度頁還顯示「還在跑」。

### W-ALERT 早就存在，只是那六支從來沒接上

這是最意外的一點。`W-ALERT 失敗推 LINE`（`UobSYUAU2C4j38tY`）從 8/28 就在，
有 errorTrigger、有 LINE 憑證。但它的便利貼寫著「**目前已接上 W-BACKUP 與
W2**」——**六支階段工作流一支都沒接**。所以 W8 死掉那晚連 LINE 都沒響。

一支工作流只能設**一個** error workflow，所以是擴充 W-ALERT，不是另建一支。

### 用 error workflow，不是加 error 出口

死的地方在迴圈深處。用節點 error 出口涵蓋，六支加起來要接幾十個，而且每新增
一個節點就多一個漏掉的機會。error workflow 是**整支執行失敗就觸發**，不管死
在哪個節點。

### /compute/chain/fail-running：比對是用 stage，不是 execution

`run` 表沒有欄位存 n8n 的執行 id，所以路由從 `workflow_id` 反查階段，再找那個
階段正在跑的 run 列：

```
一個   → 標成失敗，鏈往下走
零個   → 那是有人從表單手動跑的，本來就沒有鏈接列
多個   → 什麼都不做，回報有歧義，告警說出來
```

**最後一種是刻意拒絕的。** 為了省一次查詢就把其他專案一起標失敗，會弄髒它們
的歷史——而**錯誤的 `failed` 比卡住的 `running` 更難發現**：鏈會往前走，那個
真的跑完的階段會被重跑、重收費。
...
```
✅ 路由存在                              200
✅ 不是階段工作流 → stage: null，不是錯誤
✅ 是階段但沒在跑 → 什麼都不失敗，並說明原因
✅ 沒帶 workflow_id → 400，不是靜靜回空
```

**沒測到的只有「正好有一個在跑然後它死掉」**——那要真的弄壞一次執行才驗得到，
而它用的是同一個查詢加上 `advance(ok=false)`，後者 9/10 那晚已實測。

### ⚠️ 從外面探測這條路由一定是 404，那不是缺陷

`W-API` 的白名單**不含** `chain/fail-running`，跟 `W-RESTART` 一樣是刻意的：
修復用的內部路由不該給前端碰。而 W-ALERT 打的是**內網 URL**，根本不經過代理。

**代理說「我不認得」和後端說「沒有這條」，從外面看一模一樣。** 要驗證只能從
內網問——建一支一次性工作流打 `http://research-api.zeabur.internal:8080/...`。
...
| 工作流 | 為什麼重要 |
|---|---|
| **W-CHAIN 階段派工** | **最關鍵**——它是派工器，掛了每個專案都不再被派工，而且沒有人會知道 |
| W1 領域框架判定 | 判錯會污染後面六個階段，而且沒有任何一段會再檢查 |
| W3、W-DATA | 從自己的表單手動跑時沒有人管 |

本來就有的：W4、W5B、W6、W7、W8、W9（這次接上）、W-START、W2、W-API、W-BACKUP。

W-CHAIN 不是鏈的階段，所以 `fail-running` 會回 `stage: null` 什麼都不做，
LINE 照推——那正是想要的。

---

## ✅ PubMed 的 pmid，以及一個更糟的鄰居

`_rec` 把識別碼放進 `id`，`db._upsert_paper` 讀 `pmid`——所以每一篇 PubMed
論文的 pmid 欄位都是空的，而且不報錯（有 DOI，照樣比對去重）。

**但真正糟的是這一行：**

```python
openalex_id = r.get("openalex_id") or r.get("id")
```

`id` 是各來源自己的識別碼，所以**六個來源裡有五個**把非 OpenAlex 的東西填進
OpenAlex 欄位：pmid、arXiv id、Crossref DOI、Europe PMC id、Semantic Scholar
paperId。而 UPSERT 對這欄做 `COALESCE`，**第一個錯的值永遠留著**——一篇先從
PubMed 看到的論文，即使之後 OpenAlex 回傳真正的 id，欄位裡還是那個 pmid。

抽成 `db.openalex_id_of()`，**讓測試呼叫寫入端真正在用的那個函式**。第一版
測試自己複製了一份運算式，那種測試在真正的程式改掉之後會永遠通過。

順帶補了 Europe PMC 的 pmid（它索引 MEDLINE，回應本來就帶，也沒接住）。同一篇
論文從兩條路找到時，現在會是一列而不是兩列。

---

## ✅ 兩個新測試檔

**`tests/test_paper_identifiers.py`** — 真實 efetch XML 跑過解析器，檢查
pmid、DOI、期刊名、MeSH 都在，以及六個來源的 `openalex_id` 各自對不對。

**`tests/test_pick_screen.py`** — ③b 畫面的欄位在 node 裡跑一次。fixture 是
從 `82ffbcec` 抓的真實回應，形狀原樣保留。檢查四列都出得來、`idea_id` 併得
起來、scooped 不勾 adjacent 勾沒驗過不勾、以及 `rounds.rounds` 那個計數。

沒有 node 的機器上會跳過而不是失敗。
...
## ✅ 82ffbcec 整條鏈跑完了（2026-09-10 23:34）

```
最終花費 $2.907604 / $5.00
W2 → 採集 → W1 → W3 → W4 → W5B → W6 → W7 → W8 → W9   全部 done
```

三個方向的辯論結果，**三種結束方式各出現一次**：

| 想法 | 輪數 | 結束原因 | 怎麼讀 |
|---|---|---|---|
| 3 | 1 | 沒有帶引用的異議還開著 | ✅ 批判者拿不出有論文支撐的反對 |
| 4 | 9 | 漂移 0.56 > 0.5 | ❌ 辯到最後變成另一個題目，最後那版不採用 |
| 10 | 10 | 觸到輪數上限，異議還開著 | ⚠️ 被叫停，不是撐住 |

想法 10 的 W7 判定是 **scooped**，而且辯護方對最關鍵的兩條，一條答非所問
（批判者說「已經有人做過」，它回「所以更需要做」），一條靠把設計改成
做不到的 RCT 脫身。**那 $0.40 是白花的，而它會被勾選是設計問題——見 ③b。**

---

## ✅ 新增審閱點 ③b：分級與新穎性都齊了，再挑要辯論哪幾個

### 修的是一個順序缺陷

審閱點 ③ 要人決定哪些方向往下跑，但 **W7 新穎性在那之後才跑**——所以人是在
還不知道哪個被搶先的情況下押注。想法 10 就是這樣進到辯論的。

```
③ 選（不知道誰被搶先） → W7 新穎性 → W8 辯論 $0.40/個
```

### ⛔ 不要把 novelty 移到 feasibility 之前

我先提過這個方案，**查了之後發現不行**。W7 的 `Pick The Directions` 從
`/compute/feasibility` 讀 `statement`／`title`／`tier`：

```javascript
const b = $("The Graded Board").first().json;
all = board.filter(a => want.indexOf(a.tier) !== -1);
```

沒有分級板它會直接拋 `the board is {} - Run W6 first`。**下次有人想調換順序，
先讀這一段。**

### 實際做法：不動順序，把決定往後移

```
③ 選多一點 → W7（一個 $0.066）→ ★③b 停 → 收斂 → W8（一個 $0.40）→ ④
```

兩段選擇對應兩種成本：便宜的檢查做多一點，昂貴的辯論做少一點。

### 那個位置本來就有一個停頓，而且是我當天挖的洞

早上讓 novelty 預設暫停（為了 PubMed 補查）時，它的 `review_point` 是 `null`，
而前端路由是 `③→tiers`、`④→debate`、其他→`null`。**所以它會顯示
「停在審閱點（空白）」而且點不進去。** 沒被發現是因為 worker 41 秒就放行了。

現在它有代號 `③b`、有畫面 `screenPick`、有路由。`test_chain.py` 釘住這個代號，
**也釘住原本四個審閱點沒有被重新編號**——重新編號會無聲弄壞每一個提到它們的
畫面與文件。

### 幾個刻意的設計

- **`scooped` 預設不勾，其他預設勾。** 預設全開會讓「同意機器找到的東西」跟
  「不同意」花一樣多次點擊，而常見情況應該便宜。
- **判定旁邊帶 `coverage_limits`。** 一個標籤只能要求人相信它，涵蓋範圍說明
  才讓人查得動。
- **`releaseBlock` 的費用文案改成看審閱點。** 在 ③b 報 W7 的價錢會讓剩餘成本
  看起來比實際高，進而讓人少勾。

### worker 不再自動放行

它以前在這裡放行是對的——那時那個停頓只為了讓 PubMed 補查落地，沒有東西要決定。
**現在它是決定點，自動放行等於沿用 ③ 的盲選**，正好是這個停頓要防的事。
worker 的工作到「把資訊補齊」為止。

---

## ✅ 辯論頁現在看得到辯護方的回應

`objection` 表從建表起就有 `rebuttal`，每一輪都寫進去，而 `debate_state` 的
SELECT 只取 `o.status, o.rebuttal_score`——**沒有 `o.rebuttal`**。所以畫面顯示
攻擊、狀態、分數，就是沒有那個分數在評分什麼。

**這一個特別嚴重**：反駁分是辯護方替攻擊自己的人打的，工作流的設計本來就承認
這件事，所以它把分數分布攤出來。但讀的人只看得到分數，那個「攤出來」就沒有
意義——他還是只能選擇相信那個數字。而「這個反駁站不站得住」是唯一不能委託給
模型的判斷。

已補上 `o.rebuttal` 與 `o.cited`，前端把回應畫在攻擊底下，沒有回應的顯示
「辯護方沒有回應這一條」而不是留白——**沉默不是反駁**。

---

## 🔁 一天之內第五次：欄位寫進去了，讀的時候漏掉

| # | 欄位 | 寫入 | 讀取 |
|---|---|---|---|
| 1 | `paper.venue` | Europe PMC 的 `journalInfo.journal.title` | 讀了永遠是空的 `journalTitle` |
| 2 | `paper.mesh` | schema 第一版就有 | `_upsert_paper` 的欄位清單裡沒有 |
| 3 | `paper.pmid` | `_rec` 放進 `id` | `_upsert_paper` 讀 `pmid` |
| 4 | `objection.rebuttal` | 每一輪都寫 | `debate_state` 的 SELECT 沒選 |
| 5 | `novelty.rounds` | 是 blob | 前端猜成 `n_rounds` 計數 |

**共同點：五個都不會報錯。** 資料看起來完整、畫面看起來正常，少掉的那一半
在輸出上看不出來。

**下次寫任何讀取端的欄位名，先拿一次真實回應對過再寫。** 這五個全部是這樣
才發現的，沒有一個是靠讀程式碼看出來的。
...
## ✅ LINE 告警鏈路是通的（2026-09-10 實證，取代先前的「沒有人測過」）

昨晚這一節寫著「現在通不通沒有人測過」。**現在測過了，而且不是靠測試工作流，是
靠它真的抓到兩次失敗並送到手機上：**

```
09-10 03:30  W-BACKUP 失敗，斷在 Upload To Drive
09-10 17:46  W8 唱反調失敗，斷在 Read The Attack
```

兩則都送達。**所以 `dAytbkY0WTl24Yjm` 這把憑證是好的，W-ALERT
（`UobSYUAU2C4j38tY`）也接得對。**

先前那個「`Bearer ` 前綴掉了」的診斷是錯的，已經在下面那一節更正過。真正的
401 成因是 2026-08-28 拿 basic ID 當 User ID 用，當天就修好了。

**它證明了自己的價值**：備份掛了 11 天，是這條鏈路讓人知道的。


---

## ⚠️ W8 第一次跑就撞到輸出上限（2026-09-10 執行 2233，已修）

`82ffbcec` 放行 3 個方向之後，W8 在**第 6 輪**失敗。

### 原因

```
第 5 輪  stop_reason=end_turn   輸入 6629  輸出 3498（思考 1858）  ✅
第 6 輪  stop_reason=max_tokens 輸入 7714  輸出 6000（思考 5240）  ❌
```

**思考吃掉了 87% 的輸出預算**，只剩約 760 個 token 給 JSON，回覆在第一個異議中途
被切斷。`Read The Attack` 的搶救掃描找不到任何閉合的物件——因為確實一個都沒閉合。

**不是引用池變大造成的。** 提示詞裡本來就是 `evidence.slice(0, 20)`，所以合併讓
池子從 22 變 40 不影響提示詞。輸入從 6629 漲到 7714 是 `already_argued` 累積五輪
異議造成的，那是辯論本身的性質。
...
| | 之前 | 現在 |
|---|---|---|
| `Attack It` 的 `maxTokens` | 6000 | **12000** |
| 截斷時的錯誤訊息 | 「the critic returned no objections」 | **說出是被切斷，附上實際 token 數** |

第二項是因為那句話**害人往錯的方向追**——它聽起來像模型無話可說，實際上是預算
不夠。n8n 的 Anthropic 節點**沒有思考預算選項**，只有 `maxTokens`，所以那是唯一
的槓桿。

### 為什麼是 12000 而不是更高

第 6 輪的思考**已經跑完了**（5240），只是答案寫到 760 就被切。答案照第 5 輪的
規模約需 1600–2500，所以它實際需要約 7200–7700，12000 有約 60% 餘裕。

**不要再往上調。** 上限不是花費，但給了空間模型就會用——思考從 1858 暴增到 5240，
輸入只多了 16%。而且：

```
上限  6000  用滿 $0.076/輪   3方向×10輪 = $2.28
上限 12000  用滿 $0.136/輪                = $4.08
上限 32000  用滿 $0.336/輪                = $10.08
```

**護欄擋不住這個。** W8 的估價是 `max_rounds × max_ideas × $0.04`，跑在階段開始
之前，估完就放行——之後不管實際花多少都不會再檢查。而那個 $0.04 是照「輸出 3500
tokens」校準的。

真的再爆的話，**正確的修法是砍 `already_argued`（提示詞每輪都在長），不是繼續
加上限。**
...
```
1. 漂移超標              → 已經變成另一個方向，最後版本不採用
2. 沒有帶引用的異議還開著  → 批判者彈藥用完 = 這個方向撐住了 ✅
3. 觸到輪數上限，還有 N 個異議開著 → 被叫停，不是撐住 ⚠️
```

而且伺服器端的 `debate.MAX_ROUNDS = 10` 是**寫死的常數**，跟表單的 `max_rounds`
是兩回事：前者判斷「該不該終止」，後者只決定攤幾個 slot 給迴圈。

**把表單的 `max_rounds` 設成 6，伺服器永遠看不到第 10 輪，所以不會寫下「觸到輪數
上限」——最後一輪的 `terminated` 是 `false`。** 辯論看起來像「還沒結束」而不是
「結束了」，那比第 3 種更難判讀。

降輪數的真正代價不是「辯論比較短」，是**失去「這個方向撐住了」這個結論**。

---

## ✅ 中途死掉的階段會讓鏈永遠卡住（2026-09-12 已修，見本檔開頭）

> **這一節是問題的原始紀錄，留著是因為它說明了症狀長什麼樣。**
> 修法在開頭那一節：六支階段工作流加上 W-CHAIN 等全部指向 W-ALERT，
> 而 W-ALERT 現在會先打 `/compute/chain/fail-running` 把卡住的 run 列
> 標成失敗，再推 LINE。

W8 執行在 09:46 死了，但 `run` 那一列到 09:50 還寫著 `running`、`finished_at`
是 `None`。

**原因**：回報進度的 `Tell The Chain` 節點在**迴圈全部跑完之後**才執行。中途死掉
→ 沒有任何東西告訴鏈「這一段失敗了」。而 migration 017 不准同一階段排隊兩次，
所以 W-CHAIN 永遠不會重新派工。

**症狀最糟的地方**：鏈的外表看起來像「還在跑」，不是「失敗了」。

六個階段工作流**都是這個形狀**，不只 W8。修法大概是給每支加一個 error 出口去打
`/compute/chain/advance` 帶 `ok: false`，但那要六支都改，還沒動。

### 附帶的損失

`Lay Out All The Slots` 把 3 個方向 × 10 輪攤平成 30 個 slot **依序**跑，第 6 個
slot 死掉時全都還在第一個方向。**想法 3 和 10 一輪都沒跑到。**

好消息：已完成的 5 輪安全存著（想法 4），而 `Where It Stands` 每輪重讀狀態，
所以重跑會**從第 6 輪接下去**，不會重付前 5 輪的錢。

---

## ✅ PubMed 論文的 PMID 從來沒被存進資料庫（2026-09-12 已修，見本檔開頭）

> **留著是因為修的時候發現它旁邊還有一個更糟的：** `openalex_id` 會吃下
> `r["id"]`，所以六個來源裡有五個把非 OpenAlex 的識別碼填進那個欄位，
> 而 UPSERT 的 COALESCE 讓第一個錯的值永遠留著。詳見開頭那一節。

`_rec()` 把 PMID 放進 **`id`** 欄位，記錄裡**沒有 `pmid` 這個鍵**，而
`_upsert_paper` 讀的是 `r.get("pmid")`。

**所以每一篇來自 PubMed 的論文，`paper.pmid` 都是 NULL。** 去重只能靠 DOI，
沒有 DOI 的會落到標題比對。

跟 2026-09-10 找到的期刊名、MeSH 是**完全同一種形狀**：欄位存在、資料抓回來了、
寫入時對不上鍵名，然後安靜地丟掉。這一天找到三個。

不影響已經驗過的東西——那些論文有 DOI，識別和引用都正常。
...
### ✅ 整條新流程實跑走通了（2026-09-10 17:30–17:40）

`82ffbcec` 在審閱點 ③ 勾了 3 個方向放行，全程沒有人介入：

```
17:30:49  W7 新穎性驗證  running
17:38:23  W7            awaiting_review   ← 停在 PubMed 補查（新行為第一次生效）
17:39:04  W8 唱反調      pending          ← worker 接手、補查、合併、自動放行
17:40:26  W8            running
```
...
| 想法 | 判定 | 引用池 | 其中 PubMed |
|---|---|---|---|
| 10 | scooped | 40 篇 | **18 篇（45%）** |
| 3 | incremental | 40 篇 | **16 篇（40%）** |

**辯論的批判者現在有四成的證據來自伺服器根本抓不到的論文。** 那正是這整套
繞道要換到的東西——批判者只能引用真實檢索回來的論文，所以引用池的大小直接
決定它能不能反駁。

`novelty-pubmed-pending` 事後回 0，三筆檢查都蓋了 `pubmed_pass` 的章。

**矛盾偵測這次沒有觸發，而那是對的**：三個判定分別是 `incremental`、
`scooped`、`incremental`，沒有一個是 `no_prior_art`。那個偵測只在
「判定說什麼都沒找到，而 PubMed 在空白輪次找到了論文」時才報，其他情況只
回報篇數——要不要改判得有人讀過那幾篇。

### worker 的執行紀錄

排程模式下 stdout 沒有任何東西接住，所以前幾次無人看管的執行**完全沒留下
紀錄**。已改成同時寫 `tools/pubmed_worker.log`（超過 400 KB 自動裁到最後
2000 行，已加進 `.gitignore`）。

「沒有待辦」也會寫一行——空的紀錄檔裡，「沒事做」跟「根本沒跑」長得一模一樣，
而分辨這兩件事正是留紀錄的唯一理由。

```powershell
Get-Content D:\n8n_Claude\research-api\tools\pubmed_worker.log -Tail 20
```
...
## ⏭️ 下一步（2026-09-12 收工時的狀態）

**沒有東西卡著，沒有東西在等你。** `82ffbcec` 整條鏈跑完（$2.907604 / $5）。

### 只剩一件真正待做的

**開一個新專案，把所有修正整條走一遍。**

今天與昨天改的每一件事，都還沒在一個「從頭開始」的專案上驗過。個別都測了，
但**沒有一次完整的跑動走過新的路徑**。

新專案會不一樣的地方：

- 檢索詞可以到 5 個，超過會擋（不再默默砍成 3）
- 確認頁的 MeSH 判定跟實際檢索一致
- 只檢索最近 5 年（首頁可改）
- 採集優先挖頂刊，而且跟著領域自動換清單
- 期刊名、MeSH、**PMID** 都會被記下來
- PubMed 和 Europe PMC 同時跑（瀏覽器 + worker 兩條路）
- 概念展開不會再撈到心搏量
- **停在 ③b 讓你看著新穎性判定挑要辯論的**
- 辯論頁看得到辯護方的回應
- 任何階段死掉都會推 LINE，而且鏈會自動解開

**成本（用 `82ffbcec` 的實測數字）：**

```
W2 → 採集 → W1 → W3 → W4 → W5B → W6     $1.272562
W7 新穎性（3 個方向）                      $0.197636
─────────────────────────────────────────
跑到 ③b                                   ≈ $1.47

在 ③ 勾 8 個而不是 3 個（③b 的本意）        ≈ $1.80
之後每辯一個方向                            + $0.40
W9 報告                                    + $0.32
```

**需要的只有兩樣：研究主題，和上限。** 建議上限 $3。

停在 ③b 就可以不再花錢——它現在是要按放行的停頓，不會自己往下跑。

### 擱置中，不是忘記

**`saveExecutionProgress: true`（六支階段工作流都開著）**
就是明文金鑰那個問題，2026-09-10 已因此換過一次金鑰。**使用者決定先不關**：
關掉會看不到中途每個節點的輸出，而那正是排查「跑到一半死掉」需要的東西。
風險只在「讀執行資料」時發生，已記在長期記憶。

**NCBI 解封信** — 使用者決定不寄。PubMed 目前靠 worker 那條路走得通。

### 唯一沒被端到端驗證的一段

「**正好有一個鏈在跑，然後它死掉**」——`fail-running` 的其他三種分支都實測過，
這一種要真的弄壞一次執行才驗得到，而那會花錢。下次真的有階段死掉時看它有沒有
自己解開就知道了。

> **動手之前先讀完這一份。** 2026-09-10 有兩次是沒讀就開工：重踩了「thinking
> token 算在輸出預算裡」（W8 撞上限），以及建議把 novelty 移到 feasibility
> 之前（W7 會讀分級板，做不到）。兩條現在都在這份文件裡了。
...
## 🟡 LINE 告警：舊診斷是錯的（2026-09-10 更正，結論見檔案開頭）

這一節以前寫著「開機第一件事：LINE 掛了，`Bearer ` 前綴掉了」。**那個診斷沒有
根據。** 真正的 401 成因是 2026-08-28 拿 basic ID 當 User ID 用，當天就修好了
（工作流 `5hjjy6sjMPBJaHGg` 執行 47，三項全通、手機實際收到訊息）。

**而 2026-09-10 已經證實整條鏈路是通的**——它連續抓到 W-BACKUP 與 W8 兩次失敗
並送到手機。詳見本檔開頭那一節。
...
| `/v2/bot/info` 回的 `userId` | `Ub0ff7c128aa1fea7a4d6325392ab2300` ← **機器人自己的** |

`errorWorkflow` 只對 production 執行生效，手動測試不觸發——測告警一定要靠排程
或真的跑一次。而被指定為 errorWorkflow 的工作流**必須先 publish**。

**已經不需要做的事**：這一節以前叫人「另外建一把 `Frontend Webhook Token`」。
已經建好（`Kh0DkIvuT0Yh1oZJ`，Header 名 `X-Frontend-Key`），W-START 與 W-API
兩條 webhook 都綁它，跟 LINE 那把分開了。
...
0a-原. **外洩處理的兩個收尾**（見「🔥 2026-08-30 憑證外洩處理」）：
