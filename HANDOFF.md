...
| **W5 錦標賽** | ⚠️ 工作流已建（`CqaYzcqjNNgI05AP`，19 節點，migration 005／006 已套用、66 筆錨點入庫）。**還沒跑過任何一次**，見下方「W5 還沒驗證的四件事」 |
...
| `CqaYzcqjNNgI05AP` | **W5 錦標賽** | 19 節點。表單 `/form/w5-tournament`，`n8nUserAuth`。錯誤工作流指向 W-ALERT。**尚未跑過** |
...
工作流建好了但**一次都沒跑過**。要拿真資料驗的是：

1. `order_flip_rate` 實際落在哪。PRD 要 5–20%，這是唯一能抓到裁判位置偏誤的量。
2. 縮減場地會不會淘汰過頭。提示詞寫了「判不準時一律留下」，但沒有實測過。
3. 場次規模。15 個方向 + 8 個錨點 ≈ 460 場、150 次 Sonnet 呼叫。
   時間與費用都還沒量過，必要時調 `n_anchors`。
4. Anthropic 節點的回覆解析。W3 的 Gemini 三個節點都撞過截斷與多餘右括號，
   搶救碼照抄過來了，但 Anthropic 沒有 `jsonOutput` 開關，行為未知。

### W4 沒有真的寫進資料庫（2026-08-29 查證）

本文件先前寫著「W4 去重 ✅ 端到端通過（執行 67，20 組寫入 `dedup_pair`）」。
**查不到那 20 列。**

有方向的專案只有兩個，三次跑動全部查過：

```
cab10537 lung adenocarcinoma   run 1cfe06e8   dedup_pair 0 列
cab10537 lung adenocarcinoma   run 922e3e3d   dedup_pair 0 列
25e86e9d PM2.5 與腦中風         run a1b7e106   dedup_pair 0 列
```

其他五個專案 `n_ideas = 0`，不可能有去重配對。所以**整個資料庫沒有任何一列**。

可能是執行 67 的寫入節點根本沒被連上、或 `run_id` 沒帶到、或它回了 200 但寫了 0 列
（`save_dedup_pairs` 對 `idea_a`／`idea_b` 缺一就 `continue`，不報錯）。
**下次動 W4 時先確認寫入，不要再從執行成功推論資料存在了。**

連帶的後果：`/compute/dedup/resolve` 只驗到「端點通、查得到、回得乾淨」，
**union-find、存活者規則、人工覆寫分支都還沒有真的跑過任何一次**。
