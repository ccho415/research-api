...
## 🕳 PubMed 論文的 PMID 從來沒被存進資料庫（已知，還沒修）
...
## ⏭️ 下一步（2026-09-10 收工時的狀態）

**`82ffbcec` 整條鏈跑完了**，沒有東西卡著、沒有東西在等你。
...
**1. 開一個新專案,把今天的修正整條走一遍**

今天改的每一件事都還沒有在一個「從頭開始」的專案上驗過。新專案會不一樣的地方：
...
**2. 六支階段工作流的失敗回報（唯一還會再咬人的）**

`Tell The Chain` 都在迴圈跑完之後才執行，所以中途死掉的階段不會回報，
`run` 那列停在 `running`，而 migration 017 不准同一階段排隊兩次——鏈就永遠
卡住，**而且外表看起來像「還在跑」**。

今晚是人工建一支一次性工作流解的（`chain/advance ok:false` + `chain/start`）。
修法是給每支加一個 error 出口打 `/compute/chain/advance` 帶 `ok: false`。
六支都要改。詳見「🕳 中途死掉的階段會讓鏈永遠卡住」。

**3. `saveExecutionProgress` 存明文標頭**

今天已經因為這個換過一次後端金鑰。用到 `Research API Key` 的工作流有十幾支。

**4. PubMed 論文的 PMID 沒被存進資料庫**

一行的事，跟期刊名、MeSH 同一種形狀。不急，那些論文有 DOI，識別正常。

**5. NCBI 解封那封信**

寄 info@ncbi.nlm.nih.gov，附 IP `43.133.34.49`。解封之後 W7 直接就有 PubMed，
即時而且不需要你的機器開著——**一行程式碼都不用改**。

> **動手之前先讀完這一份。** 今天有兩次是沒讀就開工：重新踩了「thinking token
> 算在輸出預算裡」（W8 撞上限），以及建議把 novelty 移到 feasibility 之前
> （W7 會讀分級板，做不到）。兩條現在都在這份文件裡了。
