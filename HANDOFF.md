...
| **W5 錦標賽** | ✅ 滿場地實跑通過（執行 144，450 場 47 分鐘 **$2.661**）。四個守門過三個——23 場漏判待修。見下方「滿場地實跑」 |
...
模型在約 8 個批次裡漏掉 `winner` 欄位。提示詞已經寫了「EVERY verdict must contain
a winner field」，**還是會漏**。那些對局存 NULL、計分跳過，所以不會污染排名——
但 5% 的資料就這樣沒了。要修得換做法（例如逐場重試漏掉的批次），不是再加一句提示詞。
