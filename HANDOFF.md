...
## ⏭️ 下一步（2026-09-10 17:50 實測現況）

測試專案 `82ffbcec-20fc-4377-b1a1-01f5dff6061f`（雙抗血小板藥物）

```
done             W2 / 採集 / W1 / W3 / W4 / W5B / W6
done             W7 新穎性驗證   ← PubMed 補查已合併，見開頭那節
running(卡住)    W8 唱反調       ← 執行 2233 已死，run 列還寫著 running
not started      W9 最終報告

已花 $1.470198 / $3.00    剩 $1.53
```

**放行時勾了 3 個方向，但只有想法 4 辯到（5 輪，安全存著）。**
想法 3 和 10 一輪都沒跑到——見「中途死掉的階段會讓鏈永遠卡住」。

### 順序

1. **先修 Google Drive OAuth**（本檔開頭第一節）。它是唯一會讓你失去全部工作的一項。
2. 清掉 W8 卡住的 `running` 狀態，重跑 W8。`Where It Stands` 每輪重讀狀態，
   所以想法 4 會從第 6 輪接下去，不會重付前 5 輪。
3. 重跑前先想清楚額度：3 方向 × 10 輪最壞 $4.08（用滿 12000 上限才會），
   實測量級約 $0.048/輪 → 30 輪約 $1.44。剩 $1.53 是「多半夠、最壞不夠」。
   **建議調到 $5**——用不到就不花，而「這個方向撐不撐得住」重跑一次未必問得回來。
   調法：W-API 的 `budget-set`（上限 $20，多打一個零會被擋）。

### 已經做完、不用再做的

- ~~調高預算~~ ✅ $2 → $3（可能還要再調，見上）
- ~~設 Windows 排程 worker~~ ✅ 已設好並實際觸發驗過
- ~~審閱點 ③ 勾選放行~~ ✅ 勾了 3 個
- ~~驗證 W7 停在 PubMed 補查~~ ✅ 17:38:23
- ~~驗證 worker 自動接手放行~~ ✅ 17:39:04
- ~~驗證引用池真的有 PubMed 論文~~ ✅ 40 篇裡 18 篇（45%）

> **先讀完這一份再動手。** 上一個工作階段沒讀，結果重新踩了一次「thinking token
> 算在輸出預算裡」——而那條在本文件的環境備忘裡本來就寫著，2026-09-10 又在 W8
> 身上踩了第二次。
