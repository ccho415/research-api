...
| **W4 去重** | ❌ **資料庫裡一列 `dedup_pair` 都沒有**（2026-08-29 查證，見下）。而且它**只記配對、不記誰活下來**，那一半已用 `/compute/dedup/resolve` 補上 |
| **採集層** | ✅ 實跑通過，快取驗收見下 |
...
| **W5 錦標賽** | ⚠️ 端到端跑通（執行 116，60 場全判、名次已入庫），但**兩個守門發現的問題還沒解**：順序翻轉率 0.233 超標，且一個弱錨點排在強錨點之上。見下方「W5 首跑結果」 |
...
### W5 首跑結果（執行 116，2026-08-29）

冒煙測試設定：5 個方向（`max_ideas`）＋ 4 個錨點，60 場、20 批、約 7 分鐘。

```
場地   15 → 保留 15（淘汰 0）
錨點   4（強 2／弱 2）
對局   建 60，判出 60，計分 60
兩序都判過的配對 30，順序翻轉率 0.233
錨點帶 strong 1291.0 / weak 1184.7
```

**守門 2 過 1 敗。** 通過的兩個：五個方向全部落進校準帶、60 場全部判出。

#### ❌ 問題一：順序翻轉率 0.233（門檻 0.05–0.20）

30 個配對裡翻了 7 次。門檻換算成場數是 1.5–6 次，所以只超出一次。
**樣本只有 30 對，這個數字的雜訊很大，不要當成定論。** 要放大場地再量一次。

#### ❌ 問題二（比守門失敗更嚴重）：一個弱錨點排在強錨點之上

```
1. VLA 主動式超音波導航              strong   1331.6   10勝0敗
2. CGM × 腸道菌相                    weak     1277.4    8勝2敗   ← 錯位
5. 用基因變異當工具變數（MR）        strong   1250.4    7勝3敗
8. 再做一個營養素觀察性研究          weak     1092.0    1勝9敗
```

四組強弱配對裡三組排對了，**CGM（弱）贏過 MR（強）那一組是反的**。
兩條帶還是分得開（1291 對 1184.7），但重疊了。

**原因是我造成的，而且是可預測的**：CGM 那筆之所以是 weak，理由是
「已被 Zeevi et al., Cell 2015 佔據（1,931 次引用）」——這件事寫在 `evidence` 欄裡，
**而裁判看不到 `evidence`**。它只看得到那句敘述，而那句敘述本身讀起來完全合理。

換句話說：**凡是「弱在已經被做過」的錨點，裁判在結構上不可能判對**，因為判定它弱的
那個事實從來沒進到它眼前。相對地，「弱在設計本身」的錨點（營養素觀察性研究，
分不開營養素與健康行為）就排到了正確的最後段。

這一條直接推翻我先前寫的「殘留洩漏不值得修」。**要修的話有三條路，需要決定：**

1. **也對錨點跑一次機械查核**，讓它們有 `papers_before`。同時解掉洩漏。
   代價是 66 個錨點要有檢索詞，現在沒有。
2. **選錨點時排除「弱在已被發表」那一類**，只用弱在設計的。
   最省事，但把 66 筆裡能用的縮小了，而且 CGM 那筆示範的正是
   「新穎性崩塌會拉低貢獻性」——丟掉它等於丟掉那個教訓。
3. **接受帶會重疊**，把校準帶的措辭改弱（例如報「落在兩個錨點平均之間」而不是
   斷言等級）。誠實但資訊量低。

#### 縮減場地不是決定性的

同樣 15 個方向，執行 115 淘汰 1 個（`not_feasible`，理由具體），執行 116 淘汰 0 個。
拿掉 `temperature` 之後沒有任何一致性保證，**同一批輸入會給出不同的場地**。
不是錯，但報告要說得出當次淘汰了什麼、為什麼——`field_reduction` 表就是為此存在。
