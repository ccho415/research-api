...
最後更新：2026-09-02（預算護欄接完 W1–W10，上限在 W2 表單設，預設 $2）
...
**階段自動接續也建好了**——PRD 三個貫穿機制到齊（2026-09-02）。

## 🚨 但有一件事沒完成：那次部署卡住了

`2062ad0` 推上去之後，Zeabur 到 14:20 還沒部署完，`/compute/chain/plan` 一直
回 404。**開機第一件事是確認部署狀態**，然後照「階段自動接續」那一節列的
五個步驟驗（套 migration 017 → 驗端點 → 最後才啟用 W-CHAIN `UJiYf5NJRM0uVXew`）。

**W-CHAIN 目前是關的，這是對的。** API 沒驗過之前不要開——開了它每十分鐘
會去認領待辦並真的啟動階段。

**然後才是第 1 項待辦：用一個真題目跑完整條鏈。**
跑之前在 W2 表單的「預算上限」填 `2`（預設值就是 2），整條鏈就有上限了。
...
新工作流 **W-CHAIN 階段派工 `UJiYf5NJRM0uVXew`**（8 節點，**目前未啟用**）。
